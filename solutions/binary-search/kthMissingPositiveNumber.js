...
const findKthPositive = (arr, k) => {
  ...
  while (leftIdx <= rightIdx) {
    ...
    const numMissingUpThroughMidIdx = midVal - midIdx; // This is the number of positive integers missing up through (and including) `midIdx`.

    if (numMissingUpThroughMidIdx > k) {
      rightIdx = midIdx - 1;
    } else {
      leftIdx = midIdx + 1;
    }

...
const getFrequencies = (elements) => {
    const frequencies = {};

    for (const element of elements) {
        const elementInFrequencies = frequencies.hasOwnProperty(element);
        if (!elementInFrequencies) frequencies[element] = 0;
...
const topKFrequent = (nums, k) => {
    const frequencies = getFrequencies(nums);

    const kMostFrequentElements = new MinHeap([], (a, b) => frequencies[a] - frequencies[b]);
    const uniqueNums = Object.keys(frequencies);

    for (const num of uniqueNums) {
        kMostFrequentElements.push(num);

        if (kMostFrequentElements.size() > k) kMostFrequentElements.pop();
    }

    return kMostFrequentElements.getElements();
};

...
const comparePointsByDistance = (pointOne, pointTwo) => {
    const [xOne, yOne] = pointOne;
    const [xTwo, yTwo] = pointTwo;

    const pointOneDistanceFromOrigin = getDistance(xOne, yOne, 0, 0);
    const pointTwoDistanceFromOrigin = getDistance(xTwo, yTwo, 0, 0);

    return pointOneDistanceFromOrigin - pointTwoDistanceFromOrigin;
};

const kClosest = (points, k) => {
    const kClosestPoints = new MaxHeap([], comparePointsByDistance);

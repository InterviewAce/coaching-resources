...
class Solution:
    def findMin(self, nums: List[int]) -> int:
        ...
        min_element_idx = NO_MINIMUM_FOUND

        while left_idx <= right_idx:
            mid_idx = get_mid_idx(left_idx, right_idx)
            mid_val = nums[mid_idx]

            is_less_than_or_equal_to_last = mid_val <= last_val
            if is_less_than_or_equal_to_last:
                min_element_idx = mid_idx
                right_idx = mid_idx - 1
            else:
                left_idx = mid_idx + 1

        return nums[min_element_idx]
